...
import { LinkLocationComponent } from '../../../_layout/Shared/link-location/link-location.component';
...
export class BroadcastMessageComponent implements OnInit {
  ...
  constructor(
    ...
    private surveyService: SurveysService, private spinner: NgxSpinnerService, private signalRService: SignalRService,
    private route: ActivatedRoute, private helper: Helper, private broadcastMessageService: BroadcastMessageService, private translate: TranslateService,
    ...
    private localStorage: LocalStorageService, private webSocketService: WebSocketService) {
    ...
    this.duplicateMessageId = this.route.snapshot.paramMap.get('dupId');
    console.log(this.eventId + '_' + this.messageId + '_' + this.duplicateMessageId);
  }
  ...
  ngOnInit(): void {
    ...
    this.inputBroadcastMessageModel.eventId = this.eventId;
  ...
  ifMsgDurationEnabled(e) {
    if (e.target.checked) {
      this.inputBroadcastMessageModel.includeMessageDuration = true;
      this.isMsgDurationEnabled=true;

    } else {
      this.inputBroadcastMessageModel.includeMessageDuration = false;
  ...
  openLinkContentModal(index) {
    ...
    this.modalRef = this.modalService.show(OpenLinkContentModalComponent, modalConfig);
  ...
  getMessageDetails() {
    ...
    this.broadcastMessageService.getBroadcastMessageDetails(msg).subscribe((res) => {
      if (res.isSuccess) {
        ...
        this.inputBroadcastMessageModel.modifiedBy = this.localStorage.get('UserId');
  ...
  savemessage() {
    ...
    if (this.inputBroadcastMessageModel.messageId != '') {
      this.inputBroadcastMessageModel.createdBy = this.localStorage.get('UserId');
    }
    ...
    this.inputBroadcastMessageModel.messageId = AppUtils.emptyGuid;
    ...
    this.broadcastMessageService.upsertBroadcastMessage(this.inputBroadcastMessageModel).subscribe((res) => {
      if (res.isSuccess) {
        ...
        // if (this.inputBroadcastMessageModel.sendNow) {
          ...
          msg.isActive = true;
          msg.modifiedBy = this.localStorage.get('UserId');
          Swal.fire({
            title: new SweetAlertMessage(this.translate).success,
            text: new SweetAlertMessage(this.translate).savedSuccessfully,
            icon: 'success'
          }).then(() => {
            location.href = AppUtils.EventUrl + '/admin/events/broadcastmessage/' + this.eventId;
          });
          // this.webSocketService.sendSquareChangeRequest(msg);
          // this.broadcastMessageService.startSendingBroadcastMessage(msg).subscribe((res) => {
          //   if (res.isSuccess) {
          //     // this.spinner.hide();
