...
@Component({
  selector: 'app-video-watched-report',
  templateUrl: './video-watched-report.component.html',
  styleUrls: ['./video-watched-report.component.css']
})
export class VideoWatchedReportComponent implements OnInit {
  eventName: string;
  eventId: string;
  noRecordFound = false;
  ...
  public columnDefs: ColDef[] = [
    ...
    // },
    {
      headerName: "Video Title", field: 'leaderboardActivityName', sortable: true, filter: 'agMultiColumnFilter', enableRowGroup: true,
      enablePivot: true, resizable: true, flex: 1, minWidth: 200, headerClass: 'bg1', filterParams: {
        filters: [
    ...
    {
      headerName: "Start Date Time", field: 'createdDate', sortable: true, cellRenderer: (data) => {
  ...
  getVideowatched() {
    ...
    this.reportService.getVideowatched(this.getbyIdViewModel).subscribe((res) => {
      if (res.isSuccess) {
        this.spinner.hide();
