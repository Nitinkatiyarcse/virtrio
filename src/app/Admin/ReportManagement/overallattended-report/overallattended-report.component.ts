...
@Component({
  selector: 'app-overallattended-report',
  templateUrl: './overallattended-report.component.html',
  styleUrls: ['./overallattended-report.component.css']
})
export class OverallattendedReportComponent implements OnInit {
  eventName: string;
  eventId: string;
  noRecordFound = false;
 ...
 {field:'logInTime',cellRenderer: (data) => {
  return data.value ? (new Date(data.value)).toLocaleString() : ' ';
},sortable: true,enableRowGroup: true,
  enablePivot: true,filter: 'agDateColumnFilter', resizable: true,flex: 1,width:20,headerClass:'bg1',filterParams: {
    filters: [
 ...
 getOverallattended(){
   ...
   this.reportService.getOverallattended(this.getbyIdViewModel).subscribe((res) => {
    if (res.isSuccess) {  
      ...
      this.rowData=res.overallAttendedReport;
